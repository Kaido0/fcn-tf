...
class FCN8VGG:
    ...
    def build(self, rgb, train=False, num_classes=4, random_init_fc8=False,
              debug=False):
        """
    ...
    def get_conv_filter(self, name): #卷积核设置
        init = tf.constant_initializer(value=self.data_dict[name][0],
                                       dtype=tf.float32)
        shape = self.data_dict[name][0].shape
        print('Layer name: %s' % name)
        print('Layer shape: %s' % str(shape)) #  Layer shape: [7, 7, 512, 4096]
        var = tf.get_variable(name="filter", initializer=init, shape=shape) #W=[高，宽，输入通道数，输出通道数]
